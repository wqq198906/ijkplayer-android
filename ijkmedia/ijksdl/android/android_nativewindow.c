...
static int android_render_rgb_on_rgb(ANativeWindow_Buffer *out_buffer, const SDL_VoutOverlay *overlay, int bpp)
{
    ...
    // ALOGI("wqq-android_render_rgb_on_rgb(%p), bpp:%d", overlay, bpp);
...
int SDL_Android_NativeWindow_display_l(ANativeWindow *native_window, SDL_VoutOverlay *overlay)
{
    ...
    // ALOGI("wqq-SDL_Android_NativeWindow_display_l render");
    if (render_ret < 0) {
        // TODO: 8 set all black
...
int SDL_Android_Bitmap_display_l(void *pixels, void *buffer, int width, int height)
{
    if (pixels == NULL || buffer == NULL) {
        ALOGE("wqq-SDL_Android_Bitmap_display_l src:%p, dst:%p", pixels, buffer);
        return -1;
    }
    SDL_VoutOverlay *overlay = buffer;
    int min_height = IJKMIN(height, overlay->h);
    int dst_stride = width;

    // ALOGI("wqq-SDL_Android_Bitmap_display_l src:%p, dst:%p, w:%d, h:%d, min_height:%d, dst_stride:%d", pixels, buffer, width, height, min_height, dst_stride);

    if (overlay->pitches == NULL || overlay->pixels == NULL) {
        ALOGE("wqq-SDL_Android_Bitmap_display_l pitches:%p, pixels:%p", overlay->pitches, overlay->pixels);
        return -1;
    }

    int src_line_size = overlay->pitches[0];
    int dst_line_size = dst_stride * 4;

    uint8_t *dst_pixels = pixels;
    const uint8_t *src_pixels = overlay->pixels[0];

    // ALOGI("wqq-SDL_Android_Bitmap_display_l src:%p, dst:%p, w:%d, h:%d, dst_line_size:%d, src_line_size:%d", pixels, buffer, width, height, dst_line_size, src_line_size);

    if (dst_line_size == src_line_size) {
        int plane_size = src_line_size * min_height;
        // ALOGI("wqq-android_render_rgb_on_rgb (pix-match) %p %p %d", dst_pixels, src_pixels, plane_size);
        memcpy(dst_pixels, src_pixels, plane_size);
    } else {
        // TODO: 9 padding
        int bytewidth = IJKMIN(dst_line_size, src_line_size);

        // ALOGI("wqq-android_render_rgb_on_rgb (pix-mismatch) %p %d %p %d %d %d", dst_pixels, dst_line_size, src_pixels, src_line_size, bytewidth, min_height);
        av_image_copy_plane(dst_pixels, dst_line_size, src_pixels, src_line_size, bytewidth, min_height);
    }
