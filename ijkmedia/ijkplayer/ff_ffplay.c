...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    if (vp->bmp) {
        ...
        // ALOGI("wqq-SDL_VoutDisplayYUVOverlay pts:%f", vp->pts);
        ffpipeline_update_bitmap(ffp->pipeline, vp->bmp);
        is->last_render_video_time = (int)(vp->pts * 1000);
...
static void video_refresh(FFPlayer *opaque, double *remaining_time)
{
    ...
    if (is && (is->expect_render_video_time != -1)) {
        if (is->expect_render_video_time >= is->last_render_video_time) {
            // ALOGI("wqq-expect_render_video_time:%d, last_render_video_time:%d", is->expect_render_video_time, is->last_render_video_time);
            goto expect_display;
        }
        return ;
    }
...
display:
        /* display picture */
        // ALOGI("display_disable:%d, force_refresh:%d, show_mode:%d, rindex_shown:%d", ffp->display_disable, is->force_refresh, is->show_mode, is->pictq.rindex_shown);
...
void ffp_set_video_render_time(FFPlayer *ffp, int time)
{
    assert(ffp);
    if (!ffp)
        return;
    ...
    if (ffp->is) {
        ffp->is->expect_render_video_time = time;
        // ALOGI("wqq-ffp_set_video_render_time %d", time);
    }
}
