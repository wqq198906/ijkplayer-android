...
static IJKFF_Pipenode *func_open_video_decoder(IJKFF_Pipeline *pipeline, FFPlayer *ffp)
{
    ...
    if (ffp->mediacodec_all_videos || ffp->mediacodec_avc || ffp->mediacodec_hevc || ffp->mediacodec_mpeg2)
        ...
        ALOGI("wqq-ffpipenode_create_video_decoder_from_android_mediacodec");
    if (!node) {
        node = ffpipenode_create_video_decoder_from_ffplay(ffp);
        ALOGI("wqq-ffpipenode_create_video_decoder_from_ffplay");
    }
...
int ffpipeline_set_bitmap(JNIEnv *env, IJKFF_Pipeline* pipeline, jobject bitmap)
{
    ALOGD("%s()\n", __func__);
    if (!check_ffpipeline(pipeline, __func__))
        return -1;
    ...
    ffpipeline_lock_surface(pipeline);
    {
        jobject prev_bitmap = opaque->jbitmap;

        if ((bitmap == prev_bitmap) ||
            (bitmap && prev_bitmap && (*env)->IsSameObject(env, bitmap, prev_bitmap))) {
            // same object, no need to reconfigure
        } else {
            if (bitmap) {
                opaque->jbitmap = (*env)->NewGlobalRef(env, bitmap);
            } else {
                opaque->jbitmap = NULL;
            }

            if (prev_bitmap != NULL) {
                SDL_JNI_DeleteGlobalRefP(env, &prev_bitmap);
            }
        }
    }
    ffpipeline_unlock_surface(pipeline);
...
void ffpipeline_update_bitmap_l(jobject jbitmap, void *overlay) {
	JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
	...
	if (info.width <= 0 || info.height <= 0 ||
		...
		ALOGE("%s: invalid bitmap\n", __func__);
		return;
	}

	// ALOGI("%s: wqq-Effect: %dx%d, %d\n", __func__, info.width, info.height, info.format);
	// Lock the bitmap to get the buffer
	void * pixels = NULL;
	int res = AndroidBitmap_lockPixels(env, jbitmap, &pixels);
	if (pixels == NULL) {
		ALOGE("%s: fail to lock bitmap: %d\n", __func__, res);
		return;
	}

    SDL_Android_Bitmap_display_l(pixels, overlay, info.width, info.height);

	AndroidBitmap_unlockPixels(env, jbitmap);
}

void ffpipeline_androd_update_bitmap(IJKFF_Pipeline* pipeline, void *overlay)
{
    // ALOGD("%s\n", __func__);
    if (!check_ffpipeline(pipeline, __func__))
        return;
    ...
    IJKFF_Pipeline_Opaque *opaque = pipeline->opaque;
    if (opaque->jbitmap != NULL) {
        ffpipeline_update_bitmap_l(opaque->jbitmap, overlay);
    }
}