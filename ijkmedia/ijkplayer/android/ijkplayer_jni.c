...
static void
IjkMediaPlayer_setVideoBitmap(JNIEnv *env, jobject thiz, jobject jbitmap)
{
    MPTRACE("%s\n", __func__);
    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
    JNI_CHECK_GOTO(mp, env, NULL, "mpjni: setVideoBitmap: null mp", LABEL_RETURN);

    ijkmp_android_set_bitmap(env, mp, jbitmap);
...
static void
IjkMediaPlayer_setVideoRenderTime(JNIEnv *env, jobject thiz, jint time)
{
    // MPTRACE("%s\n", __func__);
    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
    JNI_CHECK_GOTO(mp, env, NULL, "mpjni: setVideoRenderTime: null mp", LABEL_RETURN);

    ijkmp_set_video_render_time(mp, time);
...
static JNINativeMethod g_methods[] = {
    ...
    { "_setVideoBitmap",       "(Landroid/graphics/Bitmap;)V", (void *) IjkMediaPlayer_setVideoBitmap },
    { "_setVideoRenderTime",    "(I)V",     (void *) IjkMediaPlayer_setVideoRenderTime },
