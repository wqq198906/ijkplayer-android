...
void ijkmp_android_set_bitmap_l(JNIEnv *env, IjkMediaPlayer *mp, jobject android_bitmap)
{
    if (!mp || !mp->ffplayer || !mp->ffplayer->vout)
        return;

    ffpipeline_set_bitmap(env, mp->ffplayer->pipeline, android_bitmap);
}

void ijkmp_android_set_bitmap(JNIEnv *env, IjkMediaPlayer *mp, jobject android_bitmap)
{
    if (!mp)
        return;

    MPTRACE("ijkmp_android_set_bitmap(bitmap=%p)", (void*)android_bitmap);
    pthread_mutex_lock(&mp->mutex);
    ijkmp_android_set_bitmap_l(env, mp, android_bitmap);
    pthread_mutex_unlock(&mp->mutex);
    MPTRACE("ijkmp_android_set_bitmap(bitmap=%p)=void", (void*)android_bitmap);
}
